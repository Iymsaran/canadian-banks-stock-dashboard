...
📈 Dashboard Features
ChartDescriptionStock Price TrendMulti-line chart showing daily closing prices for all 5 banks (2021–2026)Average Yearly Stock PriceHorizontal grouped bar chart comparing each bank's average price by yearMoving Average vs Actual PriceDual-axis line chart showing 30-day moving average vs actual closing price
...
Dataset: Top 50 Canadian Stocks Since 2010 — Kaggle (Harbhajan Singh)
...
Format conversion — Reshaped from wide format (each bank as a column) to long format (one row per bank per date) using SQL and Python
Date filtering — Filtered to 2021–2026 to focus on the post-COVID economic cycle
Null removal — Removed rows with missing closing prices
Column renaming — Standardized ticker symbols to full bank names for readability
